...
const map = {
   ...
   * @see {@link ссылка|Array.prototype.forEach()}
   */
...
const snake = {
   ...
   * @see {@link ссылка|Array.prototype.some()}
...
const game = {
  ...
  canSetDirection(direction) {
    ...
    return direction !== oppDirections[this.snake.getLastStepDirection()];
  },
  ...
  canMakeStep() {
    ...
    const isCollision = this.snake.getBody().some(function(point, index) {
      return index !== 0 && point.x === nextHeadPoint.x && point.y === nextHeadPoint.y;
    });
    return !isCollision;
  },
};
